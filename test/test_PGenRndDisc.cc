...
int main() 
{
    ...
    for ( std::vector<Parcel>::iterator vit = vflock->begin(); vit != vflock->end(); vit++ ) {
       ...
       dd = nav->distance( clon, clat, xlon, xlat );
       if ( dd > r ) {
           cerr << "Bad lon,lat,z retrieval on vector[" << i << "]: dist (" << clon << "," << clat << ", " << cz << ") to (" 
                << xlon << ", " << xlat << ", " << xz << ") is " << dd << endl;
           //exit(1);
       }
       if ( ABS(xz - cz) > thickness/2.0 ) { 
           cerr << "Bad lon,lat,z retrieval on vector[" << i << "]: vdist (" << clon << "," << clat << ", " << cz << ") to (" 
                << xlon << ", " << xlat << ", " << xz << ") is " << xz-cz << endl;
           exit(1);
       }
    }
...
exit(0);
    ...
    for ( std::list<Parcel>::iterator lit = lflock->begin(); lit != lflock->end(); lit++ ) {
       ...
       dd = nav->distance( clon, clat, xlon, xlat );
       if ( dd > r ) {
           cerr << "Bad lon,lat,z retrieval on list[" << i << "]: dist (" << clon << "," << clat << ", " << cz << ") to (" 
                << xlon << ", " << xlat << ", " << xz << ") is " << dd << endl;
           //exit(1);
       }
       if ( ABS(xz - cz) > thickness/2.0 ) { 
           cerr << "Bad lon,lat,z retrieval on list[" << i << "]: vdist (" << clon << "," << clat << ", " << cz << ") to (" 
                << xlon << ", " << xlat << ", " << xz << ") is " << xz-cz << endl;
           exit(1);
       }
    } 
    ...
    for (std::deque<Parcel>::iterator dit = dflock->begin(); dit != dflock->end(); dit++ ) {
       ...
       dd = nav->distance( clon, clat, xlon, xlat );
       if ( dd > r ) {
           cerr << "Bad lon,lat,z retrieval on deque[" << i << "]: dist (" << clon << "," << clat << ", " << cz << ") to (" 
                << xlon << ", " << xlat << ", " << xz << ") is " << dd << endl;
           //exit(1);
       }
       if ( ABS(xz - cz) > thickness/2.0 ) { 
           cerr << "Bad lon,lat,z retrieval on deque[" << i << "]: vdist (" << clon << "," << clat << ", " << cz << ") to (" 
                << xlon << ", " << xlat << ", " << xz << ") is " << xz-cz << endl;
           exit(1);
       }
    } 
    ...
    for ( Flock::iterator it = flock->begin(); it != flock->end(); it++ ) {
       ...
       dd = nav->distance( clon, clat, xlon, xlat );
       if ( dd > r ) {
           cerr << "Bad lon,lat,z retrieval on Flock[" << i << "]: dist (" << clon << "," << clat << ", " << cz << ") to (" 
                << xlon << ", " << xlat << ", " << xz << ") is " << dd << endl;
           //exit(1);
       }
       if ( ABS(xz - cz) > thickness/2.0 ) { 
           cerr << "Bad lon,lat,z retrieval on Flock[" << i << "]: vdist (" << clon << "," << clat << ", " << cz << ") to (" 
                << xlon << ", " << xlat << ", " << xz << ") is " << xz-cz << endl;
           exit(1);
       }
    } 
    ...
    for ( Swarm::iterator sit = swarm->begin(); sit != swarm->end(); sit++ ) {
       sit->getPos( &xlon, &xlat );
       xz = sit->getZ();
       dd = nav->distance( clon, clat, xlon, xlat );
       if ( dd > r ) {
           cerr << "Bad lon,lat,z retrieval on Swarm[" << i << "]: dist (" << clon << "," << clat << ", " << cz << ") to (" 
                << xlon << ", " << xlat << ", " << xz << ") is " << dd << endl;
           //exit(1);
       }
       if ( ABS(xz - cz) > thickness/2.0 ) { 
           cerr << "Bad lon,lat,z retrieval on Swarm[" << i << "]: vdist (" << clon << "," << clat << ", " << cz << ") to (" 
                << xlon << ", " << xlat << ", " << xz << ") is " << xz-cz << endl;
           exit(1);
       }
    } 
