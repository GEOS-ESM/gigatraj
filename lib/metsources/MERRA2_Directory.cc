...
MERRA2_Directory::MVarLoc::MVarLoc(const std::string Baseurl, const std::string Categ, const std::string Tag, const int Hgrid, const int Vcoord, const int Tspace, const int Tavg, const int Tbase )
{
     baseurl = Baseurl;
     categ = Categ;
     tag = Tag;
...
MERRA2_Directory::MVarLoc::~MVarLoc()
{

}

MERRA2_Directory::MVarDesc::MVarDesc(const std::string Quantity, const int Ndims, const std::string Stdname, const std::string Longname, const std::string Units, const std::string loclist )
{
     char locsep = '|';
     char partsep = ',';
     size_t grppos;
...
MERRA2_Directory::MVarDesc::~MVarDesc()
{
...
MERRA2_Directory::MERRA2_Directory()
{
...
// by a script (catalog_merra2.py).       
#include "MERRA2_Directory_Data.hh"
...
std::string MERRA2_Directory::CFName(const std::string &merra2Name ) const
{     
     try {
        return cf_names.at(merra2Name);
     } catch (const std::out_of_range& oor) {
        throw(badName());
     }   
}

std::string MERRA2_Directory::MERRA2Name(const std::string &cfName ) const
{
     try {
...
int MERRA2_Directory::LookUp(const std::string merra2Name, const std::string date
                 ...
                 ) const  
{
    ...
    item = NULLPTR;
    if ( gmao_vars.count( merra2Name ) > 0 ) {
       item = gmao_vars.at(merra2Name);
    //} catch (const std::out_of_range& cor) {
    } else {
       std::cerr << "Unknown MERRA2 quantity " << merra2Name << std::endl;
       throw (badNotFound());
    }   
           ...
           if ( url != NULL ) {
              ...
              if ( year < 1992 ) {
                 ...
                 if ( year == 2020 && month == 9 ) {
                    url0->append( "_401" );
                 } else if ( year == 2021 && (month >= 6 && month <= 9 ) ) {
                    url0->append( "_401" );
                 } else {
                    url0->append( "_400" );
                 }              
              }         
              url0->append( loc->tag.substr(10) );
              //url0->append( loc->tag );
