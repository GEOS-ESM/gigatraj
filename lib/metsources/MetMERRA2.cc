...
const gigatraj::MERRA2_Directory MetMERRA2::dir;



// default constructor
MetMERRA2::MetMERRA2() : MetGEOSPortal() 
{    
...
MetMERRA2::MetMERRA2( std::string& date ) : MetGEOSPortal() 
{
    setup_vars();   
...
void MetMERRA2::setup_gridSpecs()
{
    HGridSpec hspec;
    VGridSpec vspec;
    int i;
...
double MetMERRA2::Portal_time_nativeTo1900( double nativeTime )
{
      //return nativeTime/24.0/3600.0  + 33969.000;
      return nativeTime/24.0/60.0  + time_zero;
};

double MetMERRA2::Portal_deltime_nativeTo1900( double nativeDelTime )
{
      return nativeDelTime/24.0/60.0;
};

void MetMERRA2::reset()
{
   std::string vunits;
...
// copy constructor
MetMERRA2::MetMERRA2( const MetMERRA2&  src) : MetGEOSPortal(src)
{
}    
...
bool MetMERRA2::legalQuantity( const std::string quantity )
{
     ...
     for ( q = testquants->begin(); q != testquants->end() && status == 0; q++ ) {
         try {
            ...
            status = 1;
         } catch (MERRA2_Directory::badNotFound err) {
            status = 1;
         }
     }
...
void MetMERRA2::setOption( const std::string &name, const std::string &value )
{
}

void MetMERRA2::setOption( const std::string &name, int value )
{
    int tmp;
...
void MetMERRA2::setOption( const std::string &name, float value )
{
}

void MetMERRA2::setOption( const std::string &name, double value )
{
}

bool MetMERRA2::getOption( const std::string &name, std::string &value )
{
   value = "";
   return false;
}
...
bool MetMERRA2::getOption( const std::string &name, int &value )
{
    bool result = false;
    int tmp;
...
bool MetMERRA2::getOption( const std::string &name, float &value )
{
    bool result = false;
    value = 0.0;
    return result;
...
bool MetMERRA2::getOption( const std::string &name, double &value )
{
    bool result = false;
    value = 0.0;
    return result;
...
FilePath* MetMERRA2::cachefile( const GridField3D* item ) const
{
    FilePath* filepath;
    std::string fname;
    std::string date;
...
std::string MetMERRA2::quantityName_cf2geos(const std::string &quantity ) const
{
     
     return dir.MERRA2Name( quantity );        

}


std::string MetMERRA2::quantityName_geos2cf(const std::string &quantity ) const
{
     std::string result;
...
int MetMERRA2::setup(  const std::string quantity, const double time )
{
    std::string caltime;
    std::string subquant;
...
int MetMERRA2::setup(  const std::string quantity, const std::string &time )
{
    std::string caltime;
    int status;
    std::string *newUrl;
...
time_t MetMERRA2::expiration( const std::string &valid_at )
{     
     return 0;
}


MetMERRA2* MetMERRA2::myNew()
{
   MetMERRA2 *dup;
...
bool MetMERRA2::bracket( const std::string &quantity, const std::string &time, std::string *t1, std::string *t2)
{
    double prev, next;
    ...
    //std::cerr << "MetMERRA2::bracket:  time=" << time << " (" << caltime << "), tspace=" << tspace << ", tbase = " << tbase << std::endl;
    ...
    //std::cerr << "MetMERRA2::bracket:  ptime=" << ptime << " (" << time2Cal((ptime-basetime)) << "), xtime=" << xtime << ", ytime=" << ytime << std::endl;
    //std::cerr << "MetMERRA2::bracket:  prev=" << prev << ", ptime - prev = " << ptime - prev << std::endl;
    //std::cerr << "MetMERRA2::bracket:  floor(ytime)=" << floor(ytime) << ", ceil(ytime)=" << ceil(ytime) << ", ytime - floor(ytime) = " << ytime - floor(ytime) << std::endl;
...
bool MetMERRA2::bracket( const std::string &quantity, double time, double *t1, double *t2)
{
...
std::vector<string> *MetMERRA2::new_testQuantity( string quantity )
{
...
void MetMERRA2::set_vertical( const std::string quantity, const std::string units )
{
     set_vertical( quantity, units, NULLPTR );
}

void MetMERRA2::set_vertical( const std::string quantity, const std::string units, const std::vector<real>* levels )
{
     std::string my_units;
...
GridLatLonField3D* MetMERRA2::new_directGrid3D( const std::string quantity, const std::string time )
{
    int i,j,k;
    real lon, lat, z;
...
GridLatLonFieldSfc* MetMERRA2::new_directGridSfc( const std::string quantity, const std::string time )
{
    int i,j;
    real lon, lat;
