...
LinearVinterp* LinearVinterp::copy() const
{
   LinearVinterp* result;
   
   result = new LinearVinterp();
...
std::string LinearVinterp::id() 
{ 
  return "LinearVinterp";
}

real  LinearVinterp::interp( const std::vector<real>* griddata, const std::vector<real>* vgriddata, real z, real bad, real vbad, int dir, int debug ) const
{
     // index of the lower gridpoint
     int lowerk;
     // value of the lower gridpoint coordinate
     real lowerz;
     // value of the data at the lower gridpoint coordinate
     real lowerdat;
     // index of the upper gridpoint
     int upperk;
     // value of the upper gridpoint coordinate
     real upperz;
     // value of the data at the upper gridpoint coordinate
     real upperdat;
     // the number of vertical coordinate values
     int nzs;
     // loop index variable along the vertical coordinate
     int k;
     ...
     if ( dir > 0 ) {  // new vertical coord increases with k index
         ...
         upperz = vbad;
         upperdat = bad;
         // starting with the next-to-top point,
         // continue while: we have not gone below the bottom point.
         //             and we have not found a legit point which lies below the desired level
         ...
         // starting from the top, find the topmost index into the profile whose 
         // new vertical coordinate data point is greater than the desired surface value
         ...
         if ( lowerz != vbad && lowerdat != bad && lowerz >= z ) {
            ...
            // starting with the index just above the lowerk point,
            // go upwards (in index) looking for the first non-bad data point
            // whose vertical coord does not match the lowerz cooridnate value
...
real LinearVinterp::calc( real z, const GridField3D& grid, int i, int j, int flags ) const
{
...
std::vector<real>* LinearVinterp::calc( const std::vector<real>&  zs, const GridField3D& grid, int i, int j, int flags ) const
{
     // indices of vertical gridpoints that bracket the desired z value
     int k1, k2;
     ...
     for ( idx=0; idx<n; idx++ ) {
        ...
        if ( vals[idx*2+0] != bad && vals[idx*2+1] != bad  ) {
             ...
             results->push_back( this->minicalc( zs[idx]
                         , grid.level(ks[idx*2+0]), grid.level(ks[idx*2+1])
...
real* LinearVinterp::calc( int n, const real* zs, const GridField3D& grid, int i, int j, int flags ) const
{
     // indices of vertical gridpoints that bracket the desired z value
     int k1, k2;
     // the interpolated values
     ...
     for ( idx=0; idx<n; idx++ ) {
        ...
        if ( vals[idx*2+0] != bad && vals[idx*2+1] != bad  ) {
             ...
             // do the linear interpolation and stroe the result
             results[idx] = this->minicalc( zs[idx]
                         , grid.level(ks[idx*2+0]), grid.level(ks[idx*2+1])
                         , vals[idx*2+0], vals[idx*2+1] );
...
GridFieldSfc* LinearVinterp::surface( const real z, const GridField3D& grid, int flags ) const
{
