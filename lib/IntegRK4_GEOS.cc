...
const std::string IntegRK4_GEOS::id = "IntegRK4_GEOS";

const real gigatraj::IntegRK4_GEOS::NEARPOLE = 85.0;

IntegRK4_GEOS :: IntegRK4_GEOS()
{
    // the default is to apply no conformal adjustment
    confml = 0;
}

void IntegRK4_GEOS :: go( real &lon, real &lat, real &z, double &t, MetGEOSDistributedData *metsrc, PlanetNav *nav, double dt0 )
{
    int flags;
...
void IntegRK4_GEOS :: go( int n, real *lons, real *lats, real *zs, int *flags, double &t, MetGEOSDistributedData *metsrc, PlanetNav *nav, double dt0 )
{
    real lon, lat, z;
    ...
    if ( debug >= 100 && nuse >=1 ) {
       ...
       std::cerr << "     IntegRK4_GEOS-vector @ (" << t << "/" << dyt << ", " << plons[0] << ", " << plats[0] 
    ...
    if ( debug >= 100 && nuse >=1) {
       ...
       std::cerr << "     IntegRK4_GEOS-vector @ (" << xt << "/" << dyt << ", "  << tmplons[0] << ", " << tmplats[0] 
    ...
    if ( debug >= 100 && nuse >=1 ) {
       std::cerr << " vRel IntegRK4_GEOS-vector * (" << xt << "/" << dyt << ", "  << plons[0] << ", " << plats[0] 
    ...
    if ( debug >= 100 && nuse >=1 ) {
       dyt = metsrc->time2Cal( xt , 3);
       std::cerr << "     IntegRK4_GEOS-vector end (" << xt << "/" << dyt << ", "  << plons[0] << ", " << plats[0] 
    ...
    if ( debug >= 100 && nuse >=1 ) {
       std::cerr << "     IntegRK4_GEOS-vector result (" << xt << "/" << dyt << ", "  << tmplons[0] << ", " << tmplats[0] 
                 << ", " << tmpzs[0] << ", " << xt << ")" << std::endl;
    }
    ...
    // advance the time
    t += dt0;
    //std::cerr << "IntegRK4_GEOS END: time=" << t << ", dt0=" << dt0 << std::endl;
