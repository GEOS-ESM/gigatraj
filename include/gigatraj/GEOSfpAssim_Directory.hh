#ifndef GIGATRAJ_GEOSfpAssim_DIRECTORY_H
#define GIGATRAJ_GEOSfpAssim_DIRECTORY_H
...
/*!
\brief class that provides an inventory for GMAO GEOSfpAssim meteorological data.


*/

class GEOSfpAssim_Directory {
    ...
    public:
      ...
      */
      ...
      /*! Returns a string containing a CF-standard quantity name, given its 
          GEOSfpAssim-standard equivalent
          \param geosName a string containing the GEOSfpAssim-standard name of the quantity
      */
      std::string CFName(const std::string &geosName ) const;

      /*! Returns a string containing a GEOSfpAssim-standard quantity name, given its 
      CF-standard equivalent
          \param cfName a string containing the CF-standard name of the quantity
      */
      std::string GEOSfpAssimName(const std::string &cfName ) const;
    
    
      /*! Given a GEOSfpAssim-standard quantity name, LookUp returns
          the base URL, the category, and the file tag to be
          used in constructing the URL needd to access GEOSfpAssim data.
          
          function returns value is 0 if the lookup was successful, 1 otherwise.
          
          \param geosName a GEOSfpAssim-standard quantity name
          ...
          \param actualBaseTime returns the found time base, in minutes
