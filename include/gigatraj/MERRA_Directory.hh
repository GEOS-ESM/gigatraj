#ifndef GIGATRAJ_MERRA_DIRECTORY_H
#define GIGATRAJ_MERRA_DIRECTORY_H
...
/*!
\brief class that provides an inventory for GMAO MERRA meteorological data.


*/

class MERRA_Directory {
    ...
    public:
      ...
      */
      ...
      /*! Returns a string containing a CF-standard quantity name, given its 
          MERRA-standard equivalent
          \param merraName a string containing the MERRA-standard name of the quantity
      */
      std::string CFName(const std::string &merraName ) const;

      /*! Returns a string containing a MERRA-standard quantity name, given its 
      CF-standard equivalent
      
          \param cfName a string containing the CF-standard name of the quantity
      */
      std::string MERRAName(const std::string &cfName ) const;
    
    
      /*! Given a MERRA-standard quantity name, LookUp returns
          the base URL, the category, and the file tag to be
          used in constructing the URL needd to access MERRA data.
          
          function returns value is 0 if the lookup was successful, 1 otherwise.
          
          \param merraName a MERRA-standard quantity name
          \param date the day for which the data are desired.  This is in ISO8601 format (e.g., ``2005-07-21'')
          \param vertgrid  desired vertical grid: 0 for native model grid, 1 for pressure surfaces, -1 for don't-care
          ...
          \param timespace desired time interval/spacing, in hours
          \param timeavg desired time averaging, in hours: 0=instamtaneous, -1=don't-care )
          ...
          \param actualBaseTime returns the found base time, in minutes
          \param url returns the the url to use to access the data
          
      */
      int LookUp(const std::string merraName, const std::string date
