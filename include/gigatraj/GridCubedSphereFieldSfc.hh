#ifndef GIGATRAJ_GRIDCUBEDSPHEREFIELDSFC_H
#define GIGATRAJ_GRIDCUBEDSPHEREFIELDSFC_H
...
The GridCubedSphereFieldSfc class provides a way of holding in one object all of the
...
A subclass of the more general GridFieldSfc, GridCubedSphereFieldSfc has its two
...
class GridCubedSphereFieldSfc : public GridFieldSfc {

   typedef std::vector<real> realvec;

   /// \brief overlays the << operator for writing a GridCubedSphereFieldSSfc object
   /*! This method serializes the GridCubedSphereFieldSfc object and sends it to an ostream.
   
        \param os the output stream (ostream) object to which the serialized object is to be sent
        \param p the GridCubedSphereFieldSfc object to be serialized and output
   */
   friend std::ostream& operator<<( std::ostream& os, const GridCubedSphereFieldSfc& p);

   /// \brief overlays the >> operator for reading a GridCubedSphereFieldSfc object
   /*! This method reads a GridCubedSphereFieldSfc object's serialzied data from 
        an istream and deserializes it.
   
        \param is the input stream (istream) object from which the serialized object is to be read
        \param p the GridCubedSphereFieldSfc object to be input and deserialzied
   */
   friend std::istream& operator>>( std::istream& is, GridCubedSphereFieldSfc& p);
   ...
   public:
      ...
      */
      GridCubedSphereFieldSfc(const GridCubedSphereFieldSfc&);
      ...
      */
      ...
      /// assignment operator
      GridCubedSphereFieldSfc& operator=(const GridCubedSphereFieldSfc& src);
      ...
      */
      real longitude( const int i ) const;
      ...
      */
      ...
      /// checks for metadata compatibility with a given GridCubedSphereFieldSfc object
      /*! This mthod checks whether another GridCubedSphereFieldSfc object has the same metadata as this one.
          Metadata include the quantity name and units, and the vertical coordinate
          name and units.
          
          \param obj the GridCubedSphereFieldSfc object with which this object is to be compared

          \return true if the metadata are the same, false otherwise
      */
      bool match( const GridCubedSphereFieldSfc& obj ) const;
      ...
      */
      ...
      /*! This method creates a duplicate of the current object.
          ...
          an existing GridCubedSphereFieldSfc object (with its gridding and coordinate
...
//   protected:
      ...
      */
      ...
      /*! Given a simple index into the data in the GridCubedSphereFieldSfc object,
          this returns the two horizontal indices into the data grid that 
